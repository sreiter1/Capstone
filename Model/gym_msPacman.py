...
writer_summary = tf.compat.v1.summary.FileWriter
...
test_model_path = "D:\\ML Class\\Capstone\\reinforcement\\model\\qlearning.ckpt-1980000"
# Atari game to learn
...
summary_interval = np.floor(1000 / action_repeat).astype(int)
...
def build_dqn(num_actions, screen_buffer_size):
    ...
    Building a DQN.
    """
    inputs = tf.compat.v1.placeholder(tf.float32, [None, screen_buffer_size, 128, 8])
    # Inputs shape: [batch, channel, height, width] need to be changed into
    # shape [batch, height, width, channel]
    net = tf.transpose(inputs, [0, 2, 3, 1])
    net = tflearn.fully_connected(net, np.intc(np.floor(1024*screen_buffer_size/2)))
...
def build_graph(num_actions):
    ...
    a = tf.compat.v1.placeholder("float", [None, num_actions])
    y = tf.compat.v1.placeholder("float", [None])
    action_q_values = tf.reduce_sum(tf.multiply(q_values, a), axis=1)
...
def build_summaries():
    ...
    summary_placeholders = [tf.compat.v1.placeholder("float")
                            for i in range(len(summary_vars))]
    assign_ops = [summary_vars[i].assign(summary_placeholders[i])
                  for i in range(len(summary_vars))]
    summary_op = merge_all_summaries(key = tf.compat.v1.GraphKeys.SUMMARIES)
...
def main():
    with tf.compat.v1.Session() as session:
        ...
        saver = tf.compat.v1.train.Saver(max_to_keep=np.ceil(TMAX / checkpoint_interval).astype(int))
