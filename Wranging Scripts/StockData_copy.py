...
class getData:
    ...
    def checkApiCalls(self):
        ...
        while time.time() < self.apiCallTime + int(60/self.rate) + 1:
    ...
    def autoUpdate(self, metrics = None, olderThan = None, stockList = None, startTicker = None, missing = True, error = False, numRecords = None):
        ...
        if error:
            metricListString += " OR error_[METRICS] = 1 \n"
        if startTicker != None:
            startTicker = "".join(e for e in startTicker if e.isalpha())
            metricListString += "AND ticker_symbol >= " + startTicker + " \n"
        if stockList != None:
        ...
        if numRecords == None:
            pass
        elif isinstance(numRecords, int):
            results = results.head(numRecords)
        else:
            return -1
...
if __name__ == "__main__":
    ...
    while(complete != 0):
        try:
            complete = info.autoUpdate(metrics = ["balance"])
        except SystemExit:
            process = checkIfProcessRunning("nsv")
            if process != None:
                os.system("schtasks /run /tn killVPN")
            ...
            info.apiTotalCalls = 0
            
        
            
    info.autoUpdate(metrics = ["overview"])
    info.autoUpdate(metrics = ["cash"])
    info.autoUpdate(metrics = ["earnings"])
    info.autoUpdate(metrics = ["income"])
    info.autoUpdate(metrics = ["time"])
